...
But that is just one way, the beauty of databases is every database implementation is different.

## Postgres Page Layout

Among the database, I would like to explore how PostgreSQL store pages and provide my criticism of certain choices made. In Postgres the default page size is 8KB, and here is how it looks like.
...
It is the fact that this pointer exist allows the HOT optimization (Heap only tuple), when an update happens to a row in postgres, a new tuple is generated, if the tuple happened to fit in the same page as the old tuple, the HOT optimization changes the old item id pointer to point to the new tuple. This way indexes and other data structures can still point to the old tuple id. Very powerful.
